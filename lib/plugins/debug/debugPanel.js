...
(function () {
	...
	const Counters = me.Object.extend({
		init(stats) {
			this.stats = {};
			this.reset(stats);
		},
		...
		reset(stats) {
			const self = this;
			(stats || Object.keys(this.stats)).forEach((stat) => {
				self.stats[stat] = 0;
			});
		},
	...
	const DebugPanel = me.Renderable.extend({
		...
		init(debugToggle) {
			...
			me.event.subscribe(me.event.VIEWPORT_ONRESIZE, (w) => {
		...
		patchSystemFn() {
			...
			const _this = this;
			const bounds = new me.Rect(0, 0, 0, 0);
			...
			me.plugin.patch(me.game, 'update', function (dt) {
				const frameUpdateStartTime = window.performance.now();
			...
			me.plugin.patch(me.game, 'draw', function () {
				const frameDrawStartTime = window.performance.now();
			...
			me.plugin.patch(me.Sprite, 'draw', function (renderer) {
				...
				if (_this.visible) {
					...
					if (me.debug.renderHitBox) {
						...
						let ax = this.anchorPoint.x * bounds.width,
		...
		drawQuadTreeNode(renderer, node) {
			...
			if (node.nodes.length === 0) {
				...
				const _alpha = (node.objects.length * 0.4) / me.collision.maxChildren;
				if (_alpha > 0.0) {
					renderer.save();
		...
		drawQuadTree(renderer) {
			...
			const x = me.game.viewport.pos.x;
			const y = me.game.viewport.pos.y;
		...
		drawMemoryGraph(renderer, endX) {
			if (window.performance && window.performance.memory) {
				const usedHeap = Number.prototype.round(window.performance.memory.usedJSHeapSize / 1048576, 2);
				const totalHeap = Number.prototype.round(window.performance.memory.totalJSHeapSize / 1048576, 2);
		...
		draw(renderer) {
			...
			// fps counter
			const fps_str = `${me.timer.fps}/${me.sys.fps} fps`;
